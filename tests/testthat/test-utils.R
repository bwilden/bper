...
test_that("returns appropriate data frame", {
...
test_that("picks column with largest value", {
  arg_max_df <- arg_max_cols(df, num_cols = ncol(df))

  expect_equal(c("a", "c"), arg_max_df$max_col)
})

test_that("picks column with largest value in restricted range", {
  arg_max_df <- arg_max_cols(df, num_cols = ncol(df) - 1)

  expect_equal(c("b", "c"), arg_max_df$max_col)
})

test_that("names correct column", {
